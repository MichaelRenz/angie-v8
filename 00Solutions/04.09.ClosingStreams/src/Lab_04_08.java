...
public class Lab_04_08 {
    private static final String workingDirectory = "D:\\Courses\\Curriculum\\Java\\Lambda-Seminar\\Labs\\Code\\Solution\\04.08.ClosingStreams\\";
    private static BufferedReader testBR = null;

    private static void testClosedReader(){
        // inserted for test purposes; the read attempt must fail, since the file should already be closed
        try {
            testBR.read();
    ...
    private static Stream<String> createStreamFromFile (String filename) {
        try {
            ...
            myStream = myStream.onClose(() -> { try { in.close(); } catch(IOException ioe) { throw new UncheckedIOException(ioe); } } );
            return myStream;
